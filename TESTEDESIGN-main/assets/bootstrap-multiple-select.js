...
$.fn.BootstrapMultiselect = function (options) {
  ...
  if (options === undefined) {
    ...
    options.optgroup =
      options.optgroup !== undefined ? options.optgroup : false;
    options.reference =
      options.reference !== undefined ? options.reference : null;
    options.hAttrs =
      options.hAttrs !== undefined && options.hAttrs.length > 0
        ? options.hAttrs
        : [];
    options.sAttrs =
      options.sAttrs !== undefined && options.sAttrs.length > 0
        ? options.sAttrs
        : [];
    options.confirmAttr =
      options.confirmAttr !== undefined && options.confirmAttr.length > 0
        ? options.confirmAttr
        : [];
    options.limparAttr =
      options.limparAttr !== undefined && options.limparAttr.length > 0
        ? options.limparAttr
        : [];
  }
  ...
  if (n !== undefined) {
    ...
    var m =
      '<div id="' +
      n +
      '" data-mobile="fullscreen" class="modal fade" tabindex="-1" role="dialog">\n' +
    ...
    $(this)
      ...
      .each(function (k, v) {
        m +=
          ...
          k +
          '">' +
          (options.optgroup ? $(v).attr("label") : $(v).text()) +
          "</label>\n" +
          "                        </div>\n";
        if (options.optgroup) {
    ...
    m +=
      ...
      '                <button type="button" class="btn btn-link limpar" ' +
      options.limparAttr.join(" ") +
      ">Limpar</button>\n" +
      '                <button type="button" class="btn btn-site confirmar" data-dismiss="modal" ' +
      options.confirmAttr.join(" ") +
      ">Confirmar</button>\n" +
      "            </div>\n" +
      "        </div>\n" +
      "    </div>\n" +
    ...
    $(
      '<div class="form-group bootstrap-multiple-select bootstrap-multiple-select-' +
        n +
        '" style="position:relative;">' +
        '   <input type="text" readonly class="form-control" id="input_' +
        n +
        '" placeholder="' +
        $(this).attr("placeholder") +
        ...
        n +
        '" style="width: 100%">' +
        '   <i class="fa fa-caret-down select-icon" data-toggle="modal" data-target="#' +
        n +
        '" style="position: absolute;top: 30%;right: 10px;"></i>' +
        "</div>"
    ).appendTo($(c).closest("div"));

    $(document)
      .on("click", "#" + n + " .confirmar", function () {
        var ck = [];
        var text = "";
        var customText = "";
        var input = $("#input_" + n);
        ...
        customText = text;
        if (text.length > size) customText = text.substr(0, size) + "...";
