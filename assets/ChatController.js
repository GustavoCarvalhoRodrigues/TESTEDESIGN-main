app.controller("chatController", function ($scope, $http, $location) {
  ...
  $scope.checkChat = function (
    ...
    var path_chat =
      ...
      document.location.protocol == "https:"
        ? "https://" + $location.host() + ":8443"
        : "http://" + $location.host() + ":8080";
    const fazFetchImobiliariaNaoTemplate = !(
      codigo_imobiliaria >= 99901 && codigo_imobiliaria <= 99999
    );
    ...
    if (fazFetchImobiliariaNaoTemplate) {
      $http
        .post(path_chat + "/atendimento/chat/retornaStatusChat.asp", data)
        .then(function (response) {
          ...
          if ($scope.chatOnline == true) {
            ...
            if (configChat == "4")
              $(body).append(
                '<script src="https://globalbot.ai/init/1/' +
                  chatGlobalBotCodigo +
                  '"></script>'
              );
          }
    ...
    if ($scope.invited) {
      ...
      $("[name=form1]").on("submit", function (e) {
        var x = window.open(
          "",
          "Popup_Window",
          "toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=475,height=520,left=312,top=50"
        );
  ...
  $scope.rejeitaConvite = function () {
    ...
    $http
      .post(
        path_chat +
          "/atendimento/chat/funConvida.asp?recusar=1&php_session=" +
          $scope.php_session
      )
      .then(function (response) {
